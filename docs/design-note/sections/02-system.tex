...
\textbf{Metric: per-impression AUC.} For one impression, AUC is the chance that a randomly chosen
...
only ever judged inside one impression; pooling rewards features that merely differ between
impressions. The effect is large: article freshness scores 0.5861 pooled but 0.5087 per impression.

\textbf{Significance: paired bootstrap 95\% confidence intervals} \cite{efron1993}. To compare two
systems we resample the impressions with replacement 1,000 times, score \emph{both} systems on each
resample, and take the middle 95\% of the differences. Sharing the resample matters: two systems
scored on the same impressions make correlated errors, and pairing cancels that noise. A gain is
claimed only if its interval excludes zero. Every choice is selected on val, and test is scored
once.
