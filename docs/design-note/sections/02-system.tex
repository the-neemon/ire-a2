...
both a click and a non-click. Pooling candidates across impressions would reward features that
merely differ between impressions, and the effect is large: freshness reads 0.5861 pooled against
0.5087 per impression.
...
\cite{robertson2009,lu2024bm25s} scores an article by the rare words it shares with a query built
from the titles of the user's last $N$ clicks ($N=30$ on \eb{}, $100$ on \mind{}), with stemming on
for both languages and a Danish stopword list. The \emph{semantic} retriever takes the mean of the
user's clicked-article vectors and finds nearest neighbours by cosine with an exact FAISS
\texttt{IndexFlatIP} \cite{johnson2019faiss}. Vectors are the provided 768-dimensional
\texttt{contrastive\_vector} for \eb{} and 384-dimensional \texttt{all-MiniLM-L6-v2}
\cite{reimers2019} for \mind{}, both chosen by measurement in Assignment 1. The two scores are
standardised within each pool and mixed linearly, weight chosen on val. Each retriever keeps its
top 200. Both indexes are held in RAM.

\textbf{Stage two, the re-ranker.} A LightGBM \cite{ke2017lightgbm} gradient-boosted tree model
with the \term{LambdaRank} objective \cite{burges2010}, which learns from pairs of candidates
inside one impression weighted by how much swapping them would change nDCG, so training optimises
the ranking rather than a click probability. We measured that choice rather than assuming it
