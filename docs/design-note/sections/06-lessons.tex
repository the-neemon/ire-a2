...
printed a \emph{hit rate} (impressions with at least one clicked article retrieved) labelled
\emph{recall} (share of clicked articles retrieved); on \mind{} test they read 0.0333 and 0.0226
and reconcile exactly, $0.0333 \times 0.6775 = 0.0226$. \mind{}'s official MRR averages over all
clicks while ours credited the first, 0.2693 against 0.3108 for one ranking. And two
implementations of ``the user vector'' read different history files, which is how we found the leak
in Section~\ref{sec:q9}. Each reproduced on one dataset and not the other, because the definitions
only diverge when impressions have several clicks (28.8\% on \mind{}, 0.5\% on \eb{}). When a
number fails on exactly one dataset, check the definition first.
...
second replaced the first and the popularity check ran zero times from the day it was written.
Reading the file cannot catch that; counting collected tests can, and 12 collected against 15
written is what exposed it. Every test must now fail on an injected violation before it counts, and
how many actually run is part of what we check.

\textbf{A guard that fires on prose teaches you to delete the prose.} That same anti-gaming test
searched source as raw text and failed on \feat{src/rerank/q9.py}, whose only mention of the
lifetime columns is a comment recording that \mind{} has none. Allow-listing the file would have
passed the suite while removing the guard from a module that never read them. It now parses the
code, so a comment costs nothing and \feat{pl.col("total_inviews")} still fails. It remains a
static scan: a column name assembled at runtime would still slip past, and closing that needs a
runtime check on what the scorer reads, which we did not build.
