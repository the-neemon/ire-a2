...
about ten times more. End to end, the servable re-ranker (\EBFinalTest{} test) far outscores stage
one with lifetime popularity added (\EBLeakyTest{} test). \mind{} cannot run this comparison: its
lifetime columns are null for all 65,238 articles.
...
\bottomrule
\end{tabular}
\caption{Future-click contamination in the engagement features before the fix. Test was clean
because it comes from the later block itself.}
\label{tab:leak}
\end{table}
