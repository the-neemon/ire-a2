...
with a \emph{two-sided} mask and a different code path from the builder's binary search. Two-sided
...
window to 12\,h or unbounded without rebuilding makes both boundary tests fail. The suite is
\textbf{29 passed, 6 skipped, 0 failed}.
...
vector scored 0.6473 and 0.5506, and making one reproduce both numbers with only the history source
changed confirmed a $+$0.0967 inflation on that feature. The fix reads one block per split, covered
by a test that fails on injection. Two consequences show the leak was real. The read-time-weighted
user vector's apparent $+$0.0085 contribution fell to $+$0.0002, not significant, so its entire
value had been the leak; and test AUC \emph{rose} from 0.7429 to 0.7461, because test features were
always clean and only the model changed.
