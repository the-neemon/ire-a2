...
begin with, no publication time, no per-click history timestamps, no read time or scroll, so the
submitted system there is 11 features against \eb{}'s 22, and the click-popularity family that
carries most of the behavioural gain cannot be computed on the test period because those clicks
are the withheld labels. What remains is close to the Assignment~1 system plus exposure features,
and it lands within 0.003 of it. The behavioural axis is worth $+$0.2077 AUC on \eb{} where the
columns exist and approximately nothing on the \mind{} leaderboard where they do not; both are
stated and neither substitutes for the other.

\textbf{The offline prediction held.} We projected 0.6460 for this exact 11-feature configuration
on our own \mind{} test split, and the leaderboard returned 0.6473, a difference of $+$0.0013 on a
2.37M-impression set we have no labels for. That agreement is the strongest available evidence
that the streaming submission path preserves the scoring semantics of the offline path it
replaced, which three abandoned implementations and a silent NaN mismatch had put in doubt.
