...
Two baselines appear in this report and they are not the same thing. Here ``stage one'' is the fused retrieval score ranking the candidates directly (\EBFusedTest{} on test). The
...
On \eb{} the re-ranker is worth \EBGainTest{} AUC over fused retrieval, \EBGainTestCI{} on test, and \EBGainVal{} \EBGainValCI{} on val. The val and test intervals
...
The same architecture is worth far less on \mind{}, \MDGainTest{} \MDGainTestCI{}. Two things
compound: five of the 22 features have no source there (Section~\ref{sec:system}), so the
behavioural axis is thinner, and \mind{}'s stage one is already stronger in absolute terms
(\MDFusedTest{} against \eb{}'s \EBFusedTest{}), leaving less headroom. A smaller gain on a dataset
with less signal is a measured property of the data, so we report the contrast rather than
presenting one system that is quietly two.

One family of features did transfer, and it was the one built under duress. An earlier round of seven features moved \mind{} by nothing significant at all. The five exposure features,
