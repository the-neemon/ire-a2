...
Three deserve a sentence. \textbf{The objective matters}: a binary objective leans on absolute
freshness, while LambdaRank leans on popularity contrasts inside the impression, which is what
decides the ranking. \textbf{Recent popularity beats lifetime popularity}: counting only the last
6 hours asks ``how popular is this right now''. In isolation that moved the feature from 0.6902 to
0.7349, but inside the ranker the gain is far smaller because other features cover for it, so we
quote the ranker figure. \textbf{Fusion helps on val and hurts on test}, both significantly: the
mixing weight does not transfer across the time boundary. That is moot next to stage two, but it
